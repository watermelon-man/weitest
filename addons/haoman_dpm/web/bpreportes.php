...
$yesterday = pdo_fetchcolumn("select SUM(pay_total) from " . tablename('haoman_dpm_pay_order') . " where rid = :rid and uniacid=:uniacid and status = :status and isadmin != 1 and createtime >= {$ydtime} AND createtime <= {$ydtimet} and pay_type in(2,3,10) " , array(':rid' => $rid, ':uniacid' => $_W['uniacid'], ':status' => 2));
$yesterday = empty($yesterday) ? 0 : $yesterday;


//总收入数据
$totalSum = pdo_fetchcolumn("select SUM(pay_total) from " . tablename('haoman_dpm_pay_order') . " where rid = :rid and uniacid=:uniacid and status = :status and isadmin != 1 and pay_type in(2,3,10) " , array(':rid' => $rid, ':uniacid' => $_W['uniacid'], ':status' => 2));
$totalSum = empty($totalSum) ? 0 : $totalSum;

//已提现数据
$totalTX = pdo_fetchcolumn("select SUM(tx_total) from " . tablename('haoman_dpm_paytxlog') . " where rid = :rid and uniacid=:uniacid and tx_status=0" , array(':rid' => $rid, ':uniacid' => $_W['uniacid']));
$totalTX = empty($totalTX) ? 0 : $totalTX;

//未提现数据
$totalTXNone = pdo_fetchcolumn("select SUM(pay_total) from " . tablename('haoman_dpm_pay_order') . " where rid = :rid and uniacid=:uniacid and status = :status and isadmin != 1 and txlogid < 1 and pay_type in(2,3,10) " , array(':rid' => $rid, ':uniacid' => $_W['uniacid'], ':status' => 2));
...
$list = pdo_fetchall("select * from " . tablename('haoman_dpm_paytxlog') . " where rid = :rid and uniacid=:uniacid and tx_status=0" . $where . " order by id desc " . $limit, $params);
