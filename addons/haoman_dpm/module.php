	...
	public $tablename = 'haoman_dpm_prize';
		...
		$creditnames = array();
         ...
         if(!$yyy){
			...
			$imgName = "haomandpm".$_W['uniacid']."_".$rid;
            ...
            $imgName2 = "haomandpm_bm".$_W['uniacid']."_".$rid;
			...
			$imgName3 = "haomandpm_messages".$_W['uniacid']."_".$rid;
			...
			if(ISCUSTOM == 1 && CUSTOM_VERSION == 'DS'){//20161230定制打赏功能的
				...
				$dsimgName = "haomandpm_ds".$_W['uniacid']."_".$rid;
...
//				include $this->template('custom_ds_form');
			...
			/******************************* 20161230定制打赏功能  *********************************************/
				...
				$znlimgName = "haomandpm_znl".$_W['uniacid']."_".$rid;
...
//				include $this->template('custom_znl_form');
                ...
                if(ISCUSTOM == 1 && CUSTOM_VERSION == 'QHB'){
                    $qhb=1;
                }
...
//        file_put_contents("../addons/haoman_dpm/sign/$uniacid/$rid/sign.txt","");
		...
		$insert = array(
			...
			'qjbpic' => $_GPC['qjbpic'],
			'ziliao' =>  $_GPC['ziliao'],
			'mobile_qdstyle' =>  $_GPC['mobile_qdstyle'],
            ...
            'qhb_starttime' => strtotime($_GPC['qhb_starttime']),
			...
			'is_transparent' => intval($_GPC['is_transparent']),
		);
		...
		$insert_yyy = array(
			...
			'yyy_resultnum' => intval($_GPC['yyy_resultnum']),
		);
        $insert_xys = array(
            'rid' => $rid,
            'uniacid' => $_W['uniacid'],
        ...
        $insert_bp = array(
            ...
            'bp_bg' => $_GPC['bp_bg'],
            'bp_carousel_bg' => $_GPC['bp_carousel_bg'],
            'bp_pay' => $_GPC['bp_pay'],
		...
		if($_GPC['hb_bgcolor_tm']<0||$_GPC['hb_bgcolor_tm']>1){
            $_GPC['hb_bgcolor_tm']=1;
        }

        $insert_hbset = array(
            'rid' => $rid,
            'uniacid' => $_W['uniacid'],
            'hd_bgimg' => $_GPC['hd_bgimg'],
            'hd_bgcolor' => $_GPC['hd_bgcolor'],
            'hb_bgcolor_tm' => $_GPC['hb_bgcolor_tm'],
        );


		$id = intval($_GPC['id']);
		if (empty($id)||$id==0) {
			 pdo_insert('haoman_dpm_reply', $insert);
		} else {
        ...
        $phoneid = intval($_GPC['photoid']);  //互动设置ID，用来判断是否有记录了，如果有记录就更新记录，没有就插入记录
        if (empty($phoneid)||$phoneid==0) {
            pdo_insert('haoman_dpm_photo_setting', $insert_hbset);
        } else {
            pdo_update('haoman_dpm_photo_setting', $insert_hbset, array('id' => $phoneid));
        }
