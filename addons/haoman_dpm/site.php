...
//12.15修改过
    ...
    public function doMobileget_the_award(){
        $this->__mobile(__FUNCTION__);
    }
...
//            }
            ...
            $ret = pdo_update('haoman_dpm_pay_order', $update, array('transid'=>$transid));
            if($exits['wordimg']==1){
                $random_credit=$this->randomhongbao($hb_money,$exits['bptime'],0.5);
                $random_credit = serialize($random_credit);
            }else{
                $random_credit=0;
            }
...
//拉黑
    ...
    //新微信端上墙页面
    public function doMobilenew_messagesindex() {
        $this->__mobile(__FUNCTION__);
    }
...
//删除活动
  ...
  private function createNonceStr($length = 16) {
     ...
     * @param $ratio    浮动系数
     */
    function randomhongbao($money,$number,$ratio = 0.5){
        $res = array(); //结果数组
	...
	//获取api_ticket
   public function getCardTicket($rid,$openid,$id){
		global $_W,$_GPC;
	    $uniacid = $_W['uniacid'];
	    ...
	    if(empty($id)){
            ...
            $award = pdo_fetch("SELECT titleid FROM " . tablename('haoman_dpm_award') . " WHERE  id = :id  LIMIT 1", array( ':id' => $id));
            if(empty($award)){
                ...
                $card_rowid =1;
                $card_id = pdo_fetchcolumn("SELECT couponid FROM " . tablename('haoman_dpm_prize') . " WHERE id = :id ", array(':id' => $award['titleid']));
...
//		load()->func('communication');
    ...
    //中奖卡券通知
    public function prize_k_send_template($from_user,$mbid,$id,$awardname,$rid){
        global $_W;
        $uniacid = $_W['uniacid'];
        $url = $_W['siteroot'] .'app/'.$this->createMobileUrl('get_the_award', array('rid' => $rid,'id'=>$id));
        if($id==0){
            $url='';
        }
        $template =array(
            ...
            "data"=>array('first'=>array('value'=>"抽奖结果通知。",

                'color'=>"#743A3A",

            ),

                'keyword1'=>array('value'=>"[".$awardname."]",
