...
$reply = pdo_fetch("SELECT id,rid,uniacid,isqd,loginpassword,qdshow,isqdthemes,panzi,qjbpic,qdthemes,title,timenum,up_qrcode,3dlogo,3dlogo1,3dlogo2,3dlogo3,isyyy,is3ddaojishi,daojishinum,daojishimusic,k_templateid FROM " . tablename('haoman_dpm_reply') . " WHERE uniacid=:uniacid AND rid = :rid LIMIT 1", array(':uniacid' => $uniacid, ':rid' => $rid));
...
$list = pdo_fetchall("SELECT id,avatar,nickname,realname FROM " . tablename('haoman_dpm_fans') . " WHERE rid = :rid and uniacid = :uniacid and isbaoming =0 and avatar != '' ORDER BY id DESC limit 170 ",array(':rid'=>$rid,':uniacid'=>$uniacid));
