...
if($reply['ziliao'] ==2 || $reply['ziliao'] ==3){
    ...
    $chars ='#^13[\d]{9}$|^14[5,7]{1}\d{8}$|^15[^4]{1}\d{8}$|^17[0,6,7,8]{1}\d{8}$|^18[\d]{9}$#';
   ...
   if($reply['mobile_qdstyle']==1){
       $pw = pdo_fetch( " SELECT id FROM ".tablename('haoman_dpm_pw')." WHERE rid=:rid and title=:title and status=0",array(':rid'=>$rid,':title'=>$mobile) );
       if($pw){
           pdo_update('haoman_dpm_pw',array('status'=>1),array('id'=>$pw['id']));
