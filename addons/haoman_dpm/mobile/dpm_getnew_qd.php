...
$list = pdo_fetchall("SELECT id,avatar,nickname,qdword,realname,awardingid FROM " . tablename('haoman_dpm_fans') . " WHERE rid = :rid and uniacid = :uniacid and avatar != '' and id > {$max_id} ORDER BY id  limit 1",array(':rid'=>$rid,':uniacid'=>$uniacid));
