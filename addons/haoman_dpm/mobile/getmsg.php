...
if($op=='new'){
        $list = pdo_fetchall("SELECT * FROM " . tablename('haoman_dpm_messages') . " WHERE rid = :rid and uniacid = :uniacid and status = 1 and is_back !=1 and is_xy !=1 and id > :id  ORDER BY id desc limit 20",array(':rid'=>$rid,':uniacid'=>$uniacid,':id'=>$maxid));
        $maxid = $list[0]['id'];
