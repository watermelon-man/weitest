    ...
    public function doWebManage() {
        ...
        if ($op == 'list') {
            ...
            foreach ($list as $key => $value) {
                ...
                if ($value['prize_mode'] == 1 && $count > $value['prize_limit'])
                    $count = $value['prize_limit']; //最早成功的几人
                $list[$key]['award'] = $count;
            }
        }else if ($op == 'award') {
            $rule = pdo_fetch('select * from ' . tablename($this->modulename . '_rule') . " where rid='{$rid}'");
            if ($rule['prize_mode'] == 1)
                $condition = " limit {$rule['prize_limit']} "; //最早成功的几人
            ...
            if (pdo_update($this->modulename . "_player", array('status' => 2), array('id' => $_GPC['pid'])) === false) {
                ...
                message('发奖成功！', $this->createWebUrl('manage', array('op' => 'award', 'rid' => $rid)));
        }
    ...
    public function doMobileIndex() {
        ...
        if ($player['status'] == 1) {//已完成
            ...
            if ($rule['prize_mode'] == 1) {//最早成功的几人
                ...
                if (in_array($player['id'], array_keys($success)))
                    $award = 1;
            } else
                $award = 1;
