...
class Tech_superarticleModuleSite extends WeModuleSite {
    ...
    public function doWebMass() {
		...
		if (IMS_VERSION > 0.8) {
	    	if ($operation == 'send') {
	    		...
	    		$psize = 10;
	    		$art_id = explode('|',$_GPC['art_lists'],10);
	    		foreach ($art_id as $key => $value) {
	    			if ($key < 8) {
		    			$art_data = pdo_fetch("SELECT * FROM " . tablename('tech_superarticle_article') . " WHERE id = '{$value}'");
		    			$slist[] = array('title'=> urlencode($art_data['title']),'description'=>urlencode($art_data['desc']),'url'=>$_W['siteroot'].'app/index.php?i='.$_W['uniacid'].'&c=entry&do=detail&m=tech_superarticle&id='.$art_data['id'],'picurl'=>tomedia($art_data['thumb']));
	    			}
	    		}
				$slist = json_encode($slist);
				$slist = urldecode($slist);
				if (!empty($_GPC['openids'])) {
	    			$openidarr = explode('|',$_GPC['openids'],100);
	    			foreach ($openidarr as $key => $value) {
	    				$openids[$key]['openid'] = $value;
	    			}
				} else {
					if (!empty($_GPC['group_id'])) {
						$groupid = "%" . $_GPC['group_id'] . "%";
		    			$openids = pdo_fetchall("SELECT * FROM " . tablename('mc_mapping_fans') . " WHERE uniacid = '{$_W['uniacid']}' AND groupid LIKE '{$groupid}' ORDER BY updatetime DESC"." LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
					} else {
		    			$openids = pdo_fetchall("SELECT * FROM " . tablename('mc_mapping_fans') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY updatetime DESC"." LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
					}
				}
				$count = 0;
    			foreach ($openids as $key => $value) {
					$data = '{"touser":"' . $value['openid'] . '","msgtype":"news","news":{"articles":' . $slist . '}}';
					$res = $this->sendRes($data);
					if ($res == 0) {
						$count += 1;
					}
					usleep(rand(30,300));
    			}
    			$code = !empty($openids) ? 1 : 0;
				die(json_encode(array('code' => $code, 'data' => $count)));
	    	} elseif ($operation == 'muban') {
	    		$pindex = intval($_GPC['page']);
	    		$psize = 10;
	    		$art_id = explode('|',$_GPC['art_lists'],10);
    			$art_data = pdo_fetch("SELECT * FROM " . tablename('tech_superarticle_article') . " WHERE id = '{$art_id[0]}'");
				if (!empty($_GPC['openids'])) {
	    			$openidarr = explode('|',$_GPC['openids'],100);
	    			foreach ($openidarr as $key => $value) {
	    				$openids[$key]['openid'] = $value;
    			...
    			foreach ($openids as $key => $value) {
					...
					$data['author'] = !empty($art_data['author']) ? $art_data['author'] : $settings['appid_title'];
					$data['createtime'] = date('Y-m-d', $art_data['createtime']); 
					$data['url'] = $_W['siteroot'].'app/index.php?i='.$_W['uniacid'].'&c=entry&do=detail&m=tech_superarticle&id='.$art_data['id'];
					$res = $this->sendMuban($data);
					if ($res == 0) {
						$count += 1;
					}
					usleep(rand(30,300));
		...
		if ($operation == 'kw') {
			...
			if ($_W['isajax']) {
				if (!empty($_GPC['content_url'])) {
					...
					die(json_encode(array('code' => 1, 'message' => '采集成功，已加入文章库！'.$newid.'!')));
				} else {
					die(json_encode(array('code' => 0, 'message' => '参数请求错误')));
				}
			}
    ...
    public function doWebLocal() {
        ...
        if ($data['code'] == 1) {
			...
			if ($res1) {
				message('恭喜您，数据初始化成功！请确保已下载模板资源包，并上传至web服务器的“addons/tech_superarticle/”目录下，资源包链接：http://pan.baidu.com/s/1c1HeleK 密码：btnh', $this->createWebUrl('list'), 'success');
			} else {
				message('抱歉，数据初始化失败，正版用户请联系梦昂处理，QQ：207321553！', '', 'error');
			}
        } else {
        	message('抱歉，数据请求化失败，正版用户请联系梦昂处理，QQ：207321553！', '', 'error');
        }
	...
	public function doWebList() {
		...
		if($operation=='display'){
			...
			$categorys = pdo_fetchall("SELECT * FROM " . tablename('tech_superarticle_category') . " WHERE uniacid = '{$_W['uniacid']}' AND enabled = 1 ORDER BY id DESC");
			$localdata = pdo_fetch("SELECT * FROM " . tablename('tech_superarticle_wxstyle') . " WHERE localdata = 1 LIMIT 1");
			if (empty($localdata)) {
				message('正在初始化模块数据，请稍后，仅在模块安装好后，第一进入此处时出现！', $this->createWebUrl('local'), 'success');
			}
			if (!$categorys) {
				message('抱歉，请先创建文章分类！', $this->createWebUrl('category', array('op' => 'display')), 'error');
			}
	...
	public function doMobileList_new(){
		$settings = $this->module['config'];
		global $_W,$_GPC;
		$k = 0;
        ...
        $category = pdo_fetchall("SELECT * FROM " . tablename('tech_superarticle_category') . " WHERE uniacid = '{$_W['uniacid']}' AND enabled = 1 AND qd_enabled = 1 ORDER BY c_order DESC, id DESC LIMIT 50");
        $category1 = pdo_fetchall("SELECT * FROM " . tablename('tech_superarticle_category') . " WHERE uniacid = '{$_W['uniacid']}' AND enabled = 1 AND qd_enabled = 1 ORDER BY c_order DESC, id DESC LIMIT 5");
        $category2 = pdo_fetchall("SELECT * FROM " . tablename('tech_superarticle_category') . " WHERE uniacid = '{$_W['uniacid']}' AND enabled = 1 AND qd_enabled = 1 ORDER BY c_order DESC, id DESC LIMIT 5 , 50");
	...
	public function doMobileList(){
        ...
        $category = pdo_fetchall("SELECT * FROM " . tablename('tech_superarticle_category') . " WHERE uniacid = '{$_W['uniacid']}' AND enabled = 1 AND qd_enabled = 1 ORDER BY c_order DESC, id DESC LIMIT 4");
		include $this->template('list');
	}
	...
	public function doMobileListajax(){
        ...
        $psize = 10;
		$articles = pdo_fetchall("SELECT id,title,`desc`,thumb,createtime,author FROM ".tablename('tech_superarticle_article')." WHERE eid = '{$_W['uniacid']}' AND category_id = '{$_GPC['c_id']}' AND is_delete = 0 AND is_save = 1 ORDER BY a_order DESC, createtime DESC LIMIT ".($pindex - 1) * $psize.",{$psize}");
		foreach ($articles as $key => $value) {
			$art_infos[$key]['title'] = $value['title'];
			$art_infos[$key]['desc'] = $value['desc'];
			$art_infos[$key]['img'] = $value['thumb'];
			$art_infos[$key]['createtime'] = date('Y-m-d', $value['createtime']);
			$art_infos[$key]['author'] = !empty($value['author']) ? $value['author'] : $settings['appid_title'];
	...
	public function doMobileDetail(){		
		...
		if(!empty($ar)){
			...
			//获取此文章打赏记录
			$dashang_record2 = pdo_fetchall("select * from ".tablename('tech_superarticle_dashang')." where ar_id = :id AND status = 1 ORDER BY id DESC limit 20", array(':id' => $id));
			$i = 0;
			foreach ($dashang_record2 as $key => $value) {
				if (!empty($value['openid'])) {
					$fan_data = pdo_fetch("select * from ".tablename('tech_superarticle_member')." where openid = :openid", array(':openid' => $value['openid']));
					if (!empty($fan_data['heading'])) {
						$j = 1;
						foreach ($dashang_record as $key1 => $value1) {
							if ($fan_data['heading'] == $value1['heading']) {
								$j = 0;
							}
						}
						if ($j == 1) {
							$dashang_record[$i]['heading'] = $fan_data['heading'];
							$i = $i + 1;
						}
					}
				}
			...
			$record_count = pdo_fetchcolumn('SELECT COUNT(*) FROM ' . tablename('tech_superarticle_dashang') . " WHERE ar_id = '{$id}' AND status = 1");
			//判断当前用户是否打赏
			...
			foreach ($comments as $key => $value) {
				...
				$comments[$key]['nickname'] = $fan_data['nickname'];
				$comments[$key]['heading'] = $fan_data['heading'];
				$comments[$key]['createtime'] = $this->friendlyDate($comments[$key]['createtime'], 'mohu');
			}
			//处理作者留言
			foreach($comments as &$vo){
				if($vo['is_author']==1){
					...
					if(!empty($author_comment['comment'])){
						...
						$vo['author_comment']['createtime'] = $this->friendlyDate($author_comment['createtime'], 'mohu');
					}	
				}
			}
			unset($vo);
	...
	public function doMobileDashang(){
		...
		global $_W,$_GPC;
		setcookie("userid", $_W['openid'], time()+3600);
		$id = intval($_GPC['id']);
		if(empty($id)){
			message('非法操作！','info');
		}
	...
	public function check_member($id){
        ...
        global $_W,$_GPC;
        $openid = !empty($_W['openid']) ? $_W['openid'] : '123';//$openid = '123';
        if (!empty($openid)) {
	        $member = pdo_fetch("select * from ".tablename('tech_superarticle_member')." where eid=:eid and openid=:openid",array(':eid'=>$_W['uniacid'],':openid'=>$openid));
	        if(empty($member)){
	            load()->model('mc');
	...
	private function sendMuban($data) {
		$settings = $this->module['config'];
		global $_W,$_GPC;
		$access_token = $this->getAccessToken();
		$keyword = '';
		for ($i=1; $i < 6; $i++) { 
			if (!empty($settings['muban_k'.$i])) {
	        	$k = str_replace('#标题#', $data['title'], $settings['muban_k'.$i]);
	        	$k = str_replace('#作者#', $data['author'], $k);
	        	$k = str_replace('#时间#', $data['createtime'], $k);
				$keyword .= '"keyword'.$i.'":{"value":"'.$k.'","color":"'.$settings['muban_ys'].'"},';
			}
		}
		$data = '{"touser":"'.$data['openid'].'","template_id":"'.$settings['muban_id'].'","url":"'.$data['url'].'","data":{"first":{"value":"'.$settings['muban_header'].'","color":"'.$settings['muban_ys'].'"},'.$keyword.'"remark":{"value":"'.$settings['muban_footer'].'","color":"'.$settings['muban_ys'].'"}}}';
