...
ig.module("impact.image").defines(function() {
	ig.Image = ig.Class.extend({
		...
		failed: !1,
		loadCallback: null,
		path: _SETTINGS.STATICPATH.Path,
...
ig.module("impact.sound-handler").defines(function() {
	ig.SoundHandler = ig.Class.extend({
		...
		sfxSoundsToLoad: [{
			name: "staticSound",
			path: __STATICPATH__+"media/audio/play/static"
		}, {
			name: "openingSound",
			path:  __STATICPATH__+"media/audio/opening/opening"
		}, {
			name: "kittyopeningSound",
			path:  __STATICPATH__+"media/audio/opening/kittyopening"
		}, {
			name: "button",
			path:  __STATICPATH__+"media/audio/sfx/button"
		}, {
			name: "blop",
			path:  __STATICPATH__+"media/audio/sfx/blop"
		}, {
			name: "step",
			path:  __STATICPATH__+"media/audio/sfx/step"
		}, {
			name: "jump",
			path:  __STATICPATH__+"media/audio/sfx/jump2"
		}, {
			name: "poof",
			path:  __STATICPATH__+"media/audio/sfx/wrong"
		}, {
			name: "star",
			path:  __STATICPATH__+"media/audio/sfx/bring"
		}, {
			name: "shield",
			path:  __STATICPATH__+"media/audio/sfx/win"
		}, {
			name: "boom",
			path:  __STATICPATH__+"media/audio/sfx/boom"
		}],
		debug: !1,
		init: function() {
			this.initSfx();
	...
	ig.Jukebox = ig.Class.extend({
		init: function() {
			this.player = new jukebox.Player({
				resources: [__STATICPATH__+"media/audio/music/bgm.mp3", __STATICPATH__+"media/audio/music/bgm.ogg"],
				spritemap: {
					music: {
						start: 0,
						end: 12.985,
...
ig.module("plugins.branding.splash").requires("impact.impact", "impact.entity").defines(function() {
	...
	EntityBranding = ig.Entity.extend({
		...
		checkClickableLayer: function(b, c, d) {
			"undefined" == typeof wm && (this.doesClickableLayerExist(b) ? (ig.game.showOverlay([b]), $("#" + b).find("[href]").attr("href", c)) : this.createClickableOutboundLayer(b, c, __STATICPATH__+"media/graphics/misc/invisible.png", d))
		},
...
ig.module("game.entities.game-ui").requires("impact.entity").defines(function() {
	EntityGameUi = ig.Entity.extend({
		...
		showGameOverScreen: function() {
			...
			Playgame.setRankingScoreDesc(this.control.gameScore);
			this.control.pause();
