 ...
 * Qq: 1037483576
 */
class category_field_data
{
    public $table = 'imeepos_runner3_category_field_data';
    ...
    public function install(){
        ...
        if(!pdo_fieldexists($this->table,'field_id')){
            pdo_query("ALTER TABLE ".tablename($this->table)." ADD COLUMN `field_id` int(11) DEFAULT '0'");
        }
        if(!pdo_fieldexists($this->table,'task_id')){
            pdo_query("ALTER TABLE ".tablename($this->table)." ADD COLUMN `task_id` int(11) DEFAULT '0'");
        }
        if(!pdo_fieldexists($this->table,'value')){
            pdo_query("ALTER TABLE ".tablename($this->table)." ADD COLUMN `value` varchar(320) DEFAULT ''");
        }
    }
}