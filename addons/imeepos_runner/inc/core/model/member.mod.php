...
class member
{
    ...
    public function update(){
        ...
        if(empty($member)){
            ...
            //$data['avatar'] = isset($member['avatar'])?tomedia($member['avatar']):$user['avatar'];
            $data['uid'] = $uid;
            $data['oauth_code'] = $oauth['code'];
    ...
    public function addxinyu($to_openid,$xinyu){
        ...
        $level = M('runner_level')->getInfo($info['level_id']);
        $displayorder = intval($level['displayorder'])>0?intval($level['displayorder']):0;
        $nextlevel = M('runner_level')->getNext($displayorder);
        if(!empty($nextlevel)){
	...
	public function install(){
        ...
        if(!pdo_fieldexists($this->table,'forbid')){
			$sql = "ALTER TABLE ".tablename($this->table)." ADD COLUMN `forbid` int(4) DEFAULT '0'";
			pdo_query($sql);
		}
        ...
        if(!pdo_fieldexists($this->table,'description')){
            pdo_query("ALTER TABLE ".tablename($this->table)." ADD COLUMN `description` varchar(320) DEFAULT ''");
        }
        ...
        if(!pdo_indexexists($this->table,'INDEX_ISRUNNER')){
            pdo_query("ALTER TABLE ".tablename($this->table)." ADD INDEX INDEX_ISRUNNER ( `isrunner` )");
        }
	}
}