...
if ($op == 'sign_record') {
	...
	foreach ($list as $key => &$value){
		...
		$member_info = mc_fetch_one($value['uid']);
		$value['realname'] = !empty($member_info['realname']) ? $member_info['realname'] : $member_info['nickname'];
		if (empty($value['realname'])) {
			$openid = mc_uid2openid($value['uid']);
			$value['realname'] = $openid;
		}
	}
	unset($value);
