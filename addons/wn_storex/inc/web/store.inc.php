	...
	if (checksubmit('submit')) {
		if (COUPON_TYPE == WECHAT_COUPON && $id) {
			...
			if (is_error($status)) {
				...
				message('门店信息已提交，等待微信审核', $this->createWeburl('store'), 'success');
			}
		}
		$store_data = array(
		...
		if (!empty($id)) {
			...
			message('门店信息更新成功', $this->createWeburl('store'), 'success');
		} else {
		...
		message('门店添加成功', $this->createWeburl('store'), 'success');
	}
}
...
if ($op == 'delete') {
	...
	if (COUPON_TYPE == WECHAT_COUPON) {
		...
		if (is_error($status)) {
			message("删除本地门店数据成功<br>通过微信接口删除微信门店数据失败,请登陆微信公众平台手动删除门店<br>错误原因：{$status['message']}", $this->createWeburl('store'), 'error');
		}
	}
...
if ($op == 'import') {
	...
	if (!empty($location)) {
		...
		message('正在导入微信门店,请不要关闭浏览器...', $this->createWeburl('store', array('op' => 'import', 'begin' => $begin + 50)), 'success');
	}
	message('导入门店成功', $this->createWeburl('store'), 'success');
}
