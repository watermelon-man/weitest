...
if ($op == 'display') {
	...
	$store_lists = pdo_getall('storex_activity_stores', array('source' => COUPON_TYPE, 'uniacid' => $_W['uniacid']));
...
if ($op == 'status') {
	...
	if (!empty($storex_set)) {
		...
		$status = pdo_insert('storex_set', array('weid' => $_W['uniacid'], 'source' => intval($_GPC['status'])));
	}
